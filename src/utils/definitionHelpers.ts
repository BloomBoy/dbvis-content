...
export const DEFAULT_SLOT_NAME: [singular: string, plural: string] = [
  'Slot',
  'Slots',
];
...
export function resolveConfigurableSlotCount(
  conf: number | boolean | [min: number, max: number] | undefined,
...
export function useResolveConfigurableSlotCount(
  ...args: Parameters<typeof resolveConfigurableSlotCount>
) {
...
export function useSlotSectionTitle(
  ...args: Parameters<typeof slotSectionTitle>
) {
  // eslint-disable-next-line react-hooks/exhaustive-deps
  return useMemo(() => slotSectionTitle(...args), args);
}
export function slotTitle<ContainerData>(
  slot:
    | keyof ContainerData
...
export function useSlotTitle<ContainerData>(
  ...args: Parameters<typeof slotTitle<ContainerData>>
) {
  // eslint-disable-next-line react-hooks/exhaustive-deps
...
export function getDefaultFieldMap<Data extends {}>(
  ...
  if (subFields != null) {
    objectEntries(subFields).forEach(
      (entry) => {
        defaultData[entry[0]] = getDefaultFieldVal<Data[keyof Data & string]>(entry[1]);
      },
    );
  }
  return defaultData;
...
export function getDefaultLayoutData<
  Layout extends LayoutTypeDef<any, any, any>,
>(layout: Layout) {
  return getDefaultFieldMap(layout.subFields);
}

export function getDefaultSlotData<Layout extends LayoutTypeDef<any, any, any>>(
  layout: Layout,
) {
  return getDefaultFieldMap(layout.componentContainerFields);
}

export function createSlot<Layout extends LayoutTypeDef<any, any, any>>(
  definition: Layout,
) {
  const newSlot: ComponentContainer<
    Layout extends LayoutTypeDef<any, infer ComponentData, any>
      ? ComponentData
      : never
  > = {
    id: getId(),
